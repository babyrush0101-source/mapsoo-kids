...
function fail(message) {
  throw new Error(`Alpha.2 fixture capture rejected: ${message}`);
}
...
assert(
  ...
  'ZIP paths do not match the exact alpha.2 fixture contract',
);
...
assert(manifest.pack?.version === VERSION, 'manifest pack version must be alpha.2');
assert(manifest.pack?.generator?.version === VERSION, 'manifest generator version must be alpha.2');
...
if (replaceOutput) {
  await replaceSafeOutputDirectory(
    OUTPUT_PARENT,
    OUTPUT_DIRECTORY,
    'Alpha.2 fixture replacement must stay inside examples/packs',
  );
}
...
console.log(
  `Captured ${EXPECTED_PATHS.length} alpha.2 files from a deterministic browser ZIP (${sha256(sourceBytes)}) into ${relative(REPOSITORY_ROOT, OUTPUT_DIRECTORY)}`,
);
