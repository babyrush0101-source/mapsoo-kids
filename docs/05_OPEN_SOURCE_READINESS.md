...
> I am the primary maintainer of Mapsoo Worldsmith, an active MIT-licensed, local-first pipeline for deterministic 2D world asset packs. Six public prereleases provide versioned PNG/JSON formats, provenance receipts, reproducible builds, a separately installed Godot importer, and Linux/Windows tests on Godot 4.3/4.7. The project aims to give indie developers an open, engine-neutral alternative for creating and auditing reusable world assets. External adoption is still early.

### API credits usage（仅在申请时使用，500 字符内候选稿）

> API credits would be used only for opt-in experiments on a replaceable image-provider adapter: generating small 2D world-asset candidates, recording model and provenance metadata, testing deterministic post-processing, and evaluating license-safe export receipts. The existing offline procedural provider remains the free baseline. No user content would be uploaded by default, and no private STOYO data would be included.

### Anything else（500 字符内候选稿）

> The repository has public CI, six immutable prereleases, reproducible release ledgers, a live browser demo, a Godot addon, security and contribution policies, and a structured first-import feedback path. We do not claim external adoption yet: maintainer and CI downloads are excluded from community evidence, itch.io publishing is postponed, and STOYO is documented only as a planned consumer until a real integration can be shown.
