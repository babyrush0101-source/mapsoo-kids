...
当前开发分支已经达到本地 portable alpha：三种世界配方、确定性生成、Canvas 预览、基础校验和 PNG + JSON ZIP 可运行；Godot importer、在线 Demo、公开 release 和外部使用证据仍未完成。因此现在适合继续积累公开成果，不适合把 alpha 描述成成熟的 Godot 一键管线。
...
Mapsoo Worldsmith is an open-source, local-first pipeline that turns a versioned world specification into portable, validated 2D asset packs. Its current alpha combines deterministic procedural generation, engine-neutral PNG/JSON manifests, and license provenance; the planned Godot importer will derive engine-native resources inside Godot instead of making the browser emit fragile editor files.
