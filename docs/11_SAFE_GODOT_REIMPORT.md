# Godot 安全重导入合同（alpha.3+）

本文描述 `Mapsoo Pack Importer` 从 Alpha.3 起沿用的派生资源所有权、重导入状态机与事务边界。当前 `v0.1.0-alpha.5` 已作为 GitHub prerelease 公开，示例包和 importer 可从[正式发布页](https://github.com/babyrush0101-source/mapsoo-kids/releases/tag/v0.1.0-alpha.5)下载；12 个远端附件的 SHA-256 已全部固定在发布注册表中。

Alpha.5 沿用同一三文件受管目录和事务状态机。新增的 `runtime/places.json` 仍是源 pack 中的权威数据，不会被复制进受管目录；它的哈希、结构和 World Spec 投影在 staging 前完成校验，生成的 Places/Marker2D 内容则由 `.tscn` 哈希纳入既有所有权基线。因此旧 Alpha.4 导入仍可读取；切换到 Alpha.5 importer 时，干净的旧输出会按现有 `updated` 路径完整重建，手工修改过的输出仍返回 `conflict`。
...
`godot/tests/import_pack_cli.gd` 对真实固定哈希发布包执行 `created → unchanged`。公开 PR 与 tag release CI 已在 Linux 和 Windows 上分别运行 Godot 4.3 与 4.7；Alpha.4 的四个 exact-pack 导入任务均已通过。
