# Godot 安全重导入合同（v0.1.0-alpha.3）

本文描述 `Mapsoo Pack Importer` 的派生资源所有权、重导入状态机与事务边界。`v0.1.0-alpha.3` 已作为 GitHub prerelease 公开，示例包和 importer 可从[正式发布页](https://github.com/babyrush0101-source/mapsoo-kids/releases/tag/v0.1.0-alpha.3)下载；11 个远端附件的 SHA-256 已全部固定在发布注册表中。
...
`godot/tests/import_pack_cli.gd` 对真实固定哈希发布包执行 `created → unchanged`。公开 PR 与 tag release CI 已在 Linux 和 Windows 上分别运行 Godot 4.3 与 4.7；alpha.3 的四个 exact-pack 导入任务均已通过。
