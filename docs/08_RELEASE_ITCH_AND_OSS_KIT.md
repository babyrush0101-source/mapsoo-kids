...
- 当前本地候选的 `pnpm check` 已通过 100 个测试、31 个跨 alpha.1/alpha.2 receipt 篡改拒绝案例、历史公开哈希复核、六张发布图验证和生产构建；
...
- [Draft PR #24](https://github.com/babyrush0101-source/mapsoo-kids/pull/24) 当前公开远端 head `c88ed63` 已完成 [check job](https://github.com/babyrush0101-source/mapsoo-kids/actions/runs/29663108914/job/88128987511)；该次运行中的真实 Chrome exporter、远端 alpha.1 附件、当时的 80 个测试、双 Vite release 与 itch 套件门禁均通过，但它不证明尚未推送的当前 100 测试候选；
- [Godot 4.3 job](https://github.com/babyrush0101-source/mapsoo-kids/actions/runs/29663108914/job/88129058005) 与 [Godot 4.7 job](https://github.com/babyrush0101-source/mapsoo-kids/actions/runs/29663108914/job/88129057998) 都下载 check job 上传的同一个固定哈希 12 文件 ZIP，并通过精确输出 `MAPSOO_PACK_CLI_OK pack_id=sunny-meadow cells=384 props=29`；
- 上述 URL 证明 candidate 的 exact-pack 流程，不证明 alpha.2 已经合并、打 tag、发布 GitHub prerelease 或上线 itch.io。
...
alpha.2 的宣传图统一使用“CI-gated”措辞，不把静态图片当执行证明。当前已公开 candidate 的权威证明是上述远端 head `c88ed63` 的 GitHub Actions job URL；它仍不证明尚未推送的本地 100 测试候选。合并与 tag 后仍须以对应新提交和 release workflow 的 job URL 为准。
...
| Godot 4.3/4.7 兼容 | 公开 main 的两个基础 smoke jobs 已通过；alpha.2 当前公开远端 head `c88ed63` 的 exact-pack jobs 也已通过，但候选尚未发布 | [candidate check](https://github.com/babyrush0101-source/mapsoo-kids/actions/runs/29663108914/job/88128987511)、[4.3 exact-pack](https://github.com/babyrush0101-source/mapsoo-kids/actions/runs/29663108914/job/88129058005)、[4.7 exact-pack](https://github.com/babyrush0101-source/mapsoo-kids/actions/runs/29663108914/job/88129057998) |
