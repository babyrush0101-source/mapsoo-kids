...
STOYO world version、scene ID、公共场景标签和年龄分类并不是核心 World Spec 的原生字段。它们由独立的 `StoyoAssetRequest` 白名单投影后写入 `dev.stoyo.assetrequest.v1`。Alpha.5 的 World Spec 0.2 已支持作者显式声明通用地点锚点，但 STOYO Asset Request 1.0 不会把 `requiredSceneTags` 自动推断成地点；道路语义、角色视觉引用和任务交互仍是后续契约。
...
| Mapsoo → STOYO | pack ID/version、Asset Request hash、manifest 逐文件 hash、整体 ZIP hash、World Spec/receipt hash、当前 tile/prop ID、atlas 坐标、地图图层、预览与 license/provenance；Alpha.7 兼容 pack 包含通用 places/structures sidecar，但尚无 STOYO 消费端映射 | Mapsoo 不生成或推断儿童画像、家庭结论、内容安全结论和打印履约数据 |
...
- [`integrations/stoyo/stoyo-mapsoo-export-receipt.schema.json`](../integrations/stoyo/stoyo-mapsoo-export-receipt.schema.json)：严格的请求到 ZIP 外部回执；
- [`scripts/export-stoyo-pack.mjs`](../scripts/export-stoyo-pack.mjs)：显式时间、无 UI、可复现、永不覆盖的 Alpha.7 兼容包导出 CLI；
- [`scripts/run-stoyo-pack-godot.ps1`](../scripts/run-stoyo-pack-godot.ps1)：逐文件摘要核验后执行真实 Godot exact-pack 导入。

```bash
pnpm exec vitest run src/integrations/stoyo/asset-request.test.ts src/adapters/import-stoyo-asset-request.test.ts
pnpm stoyo:export:verify
```
...
Alpha.5 已为作者声明的稳定语义 ID 提供通用地点合同，Alpha.6 增加地点关联建筑外观，Alpha.7 发布三个可选世界包。Alpha.8 CLI 对当前 `StoyoAssetRequest 1.0` 刻意不从标签推断地点或建筑，因此合成请求示例的 places/structures 数量仍为零。重新生成可以改变 atlas 排列或地图细节，但不能静默重用或改变已发布语义 ID；未来 STOYO 消费端应只引用语义 ID，不引用随机坐标。当前没有 STOYO 生产接入证据。
...
- importer 会验证通用 `runtime/places.json` 并派生 `Marker2D` 元数据，但尚未生成供 STOYO 运行时直接读取的专用 integration sidecar；
