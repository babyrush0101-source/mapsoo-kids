...
| Mapsoo → STOYO | pack ID/version、Asset Request hash、manifest 逐文件 hash、当前 tile/prop ID、atlas 坐标、地图图层、预览与 license/provenance；Alpha.5 pack 可包含通用 `runtime/places.json`，但尚无 STOYO 消费端映射；仍不包含整体 ZIP hash或独立 validation sidecar | Mapsoo 不生成或推断儿童画像、家庭结论、内容安全结论和打印履约数据 |
...
Alpha.5 已为 `riverbank`、`old-bridge`、`observation-point` 这类作者声明的稳定语义 ID 提供通用地点合同。重新生成可以改变 atlas 排列或地图细节，但不能静默重用或改变已发布语义 ID；未来 STOYO 消费端应只引用语义 ID，不引用随机坐标。当前没有 STOYO 生产接入证据。
...
- `requiredSceneTags` 当前随 World Spec/pack 保存，但不会自动映射成 Alpha.5 地点；地点必须由 World Spec 0.2 作者显式声明，且不代表碰撞、导航、任务或交互语义；
