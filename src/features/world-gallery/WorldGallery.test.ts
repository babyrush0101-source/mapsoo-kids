...
describe('WorldGallery', () => {
  it('renders all three published examples with explicit public and active states', () => {
    ...
    expect(html).toContain('Alpha.7 public world gallery');
    expect(html).toContain('Sunny Meadow');
    expect(html).toContain('Dustwind Outpost');
    expect(html).toContain('Frostwatch Vale');
    expect(html).toContain('published as independently verified Godot asset packs');
  ...
  it('marks the selected published example as loading and disables every action during generation', () => {
    const html = renderToStaticMarkup(createElement(WorldGallery, {
      examples: WORLD_EXAMPLES,
