...
export const ALPHA3_PACK_VERSION = '0.1.0-alpha.3' as const;

export interface BuildAlpha3PackManifestInput {
...
function parseReceiptBytes(bytes: Uint8Array): unknown {
  try {
    ...
    throw new Error('The alpha.3 receipt payload must be strict UTF-8 JSON.');
  }
}
...
/**
 * Builds the alpha.3 manifest from the already serialized payload records and
 ...
 */
export async function buildAlpha3PackManifest({
  run,
  files,
  receiptBytes,
}: BuildAlpha3PackManifestInput): Promise<PackManifest> {
  ...
  if (
    ...
    throw new Error('The alpha.3 receipt file record does not match the shipped receipt bytes.');
  }
  ...
  if (issues.length > 0) {
    throw new Error(`Receipt cannot authorize the alpha.3 manifest: ${issues.map((issue) => issue.code).join(', ')}.`);
  }
  ...
  if (!structurallyEqual(receipt, expectedReceipt)) {
    throw new Error('The alpha.3 receipt is not an exact projection of the runner-owned evidence.');
  }
  ...
  if (!bytesEqual(receiptSnapshot, canonicalReceiptBytes)) {
    throw new Error('The alpha.3 receipt payload is not the canonical receipt serialization.');
  }
  ...
  if (provenanceIssues.length > 0) {
    throw new Error(
      `Receipt cannot authorize the alpha.3 manifest: ${provenanceIssues.map((issue) => issue.code).join(', ')}.`,
    );
  }
  const outputLicense = receipt.licensing.output;
  if (outputLicense.id !== 'CC0-1.0') {
    throw new Error('The alpha.3 Godot pack schema currently supports only CC0-1.0 assets.');
  }
